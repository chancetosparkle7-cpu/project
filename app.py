...
def badge(text: str, kind: str = "ok"):
    colors = {"ok": "#e6f7ff", "warn": "#fff7e6", "over": "#e6fffb"}
    border  = {"ok": "#91d5ff", "warn": "#ffd591", "over": "#87e8de"}
    return f"""
...
def render_tracker_page():
    ...
    with c1:
        ...
        if running:
            cat = running["category"]; start = datetime.fromisoformat(running["start_iso"])
            ...
            if st.button("🛑 종료/기록"):
                ...
                write_state(None); st.session_state.running = None
    ...
    if period_mode == "주":
        ...
        if end_p <= start_p:
            st.warning("종료일은 시작일 이후여야 합니다."); return

    by_cat = summarize_minutes_by_cat(df, start_p, end_p)
    goals = load_goals()
    goal_map = goals["weekly"] if period_mode == "주" else (goals["monthly"] if period_mode == "월" else None)

    cats = sorted(load_categories())
    any_data = False
    for c in cats:
        cur_min = int(by_cat.get(c, 0)); cur_hr = cur_min / 60.0
        if goal_map is None:
            any_data = any_data or (cur_min > 0)
            st.markdown(f"- **{c}**: {cur_hr:.1f}시간")
            st.progress(min(1.0, cur_min / (60.0 if cur_min>0 else 1.0)), text=f"{cur_hr:.1f}h")
        else:
            target_min = int(goal_map.get(c, 0)); target_hr = target_min / 60.0
            ...
            if target_min <= 0:
                ...
                ratio = cur_min / target_min; pct = min(1.0, ratio)
                if ratio >= 1.0:
                    b = badge("목표 달성! 🔥","over")
                elif ratio >= .7:
                    b = badge("좋아요 👍","ok")
...
def render_reminder_page():
    ...
    # ---- 안전한 정렬 → rename (KeyError 방지) ----
    df = load_reminders()
    if df.empty:
        st.info("등록된 리마인더가 없습니다.")
    else:
        base_cols = ["title","category","due_iso","advance_minutes","repeat","active","last_fired_iso"]
        v = df.reindex(columns=base_cols)
        v = v.sort_values(["active","due_iso"], ascending=[False, True], kind="mergesort")
        v = v.rename(columns={
            "title": "제목",
            "category": "카테고리",
            "due_iso": "기한(KST)",
            "advance_minutes": "사전알림(분)",
            "repeat": "반복",
            "active": "활성",
            "last_fired_iso": "마지막 발송(KST)"
        })
...
def scan_and_fire():
    ...
    for _, row in df.iterrows():
        ...
        if should_fire_row(rowd, now_dt):
            title = rowd["title"]; due = rowd["due_iso"]; adv = int(rowd.get("advance_minutes",0))
            when = "마감 임박" if now_dt < due else "마감 도래"
            body = f"{when} · 기한: {due}\n사전알림: {adv}분\n메모: {rowd.get('note','')}"
            st.toast(f"🔔 {title}\n{body}")
            if send_slack(title, body): st.info(f"Slack 전송: {title}")
            df = mark_fired_and_roll(df, rowd["id"], now_dt); fired_any = True
    if fired_any: save_reminders(df)
...
if page == PAGE_TRACKER:
    ...
    with t1:
        ...
        if st.button("주간 목표 저장"):
            ...
            save_goals(goals); st.sidebar.success("주간 목표 저장 완료")
    with t2:
        monthly_hours = {}
        for c in sorted(load_categories()):
        ...
        if st.button("월간 목표 저장"):
            ...
            save_goals(goals); st.sidebar.success("월간 목표 저장 완료")
    st.sidebar.divider()

# (설정/데이터: 카테고리 관리) → 트래커에서만 표시
if page == PAGE_TRACKER:
